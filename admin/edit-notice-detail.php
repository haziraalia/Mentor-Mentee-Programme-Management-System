...
if (strlen($_SESSION['sturecmsaid'] == 0)) {
  ...
  if (isset($_POST['submit'])) {
    ...
    $eid = $_GET['editid'];
    $sql = "update tblnotice set NoticeTitle=:grouptitle, NoticeMsg=:groupmsg where ID=:eid";
    $query = $dbh->prepare($sql);
    $query->bindParam(':grouptitle', $grouptitle, PDO::PARAM_STR);
    $query->bindParam(':groupmsg', $groupmsg, PDO::PARAM_STR);
  ...
  <head>
    ...
    <title>MMP Management System || Update Notice</title>
    <!-- plugins:css -->
                      ...
                      if ($query->rowCount() > 0) {
                        foreach ($results as $row) { ?>
                          <div class="form-group">
                            ...
                            <input type="text" name="grouptitle" value="<?php echo htmlentities($row->NoticeTitle); ?>"
                              class="form-control" required='true'>
                          </div>
                          <div class="form-group">
                            <label for="exampleInputName1">Notice Message</label>
                            <textarea name="groupmsg" value="" class="form-control"
