...
if (strlen($_SESSION['sturecmsaid'] == 0)) {
    ...
    if (isset($_GET['delid'])) {
        ...
        $sql = "delete from tblappoinment where ID=:rid";
                                            ...
                                            <table class="table">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th class="font-weight-bold">Group No</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    $ret = "SELECT ID FROM tblappoinment";
                                                    $query1 = $dbh->prepare($ret);
                                                    $query1->execute();
                                                    $results1 = $query1->fetchAll(PDO::FETCH_OBJ);
                                                    $total_rows = $query1->rowCount();
                                                    $total_pages = ceil($total_rows / $no_of_records_per_page);
                                                    $sql = "SELECT * from tblappoinment ORDER BY GroupNo ASC LIMIT $offset, $no_of_records_per_page";
                                                    $query = $dbh->prepare($sql);
                                                    $query->execute();
                                                    ...
                                                    if ($query->rowCount() > 0) {
                                                        foreach ($results as $row) { ?>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <?php echo htmlentities($row->GroupNo); ?>
                                                                </td>
                                                                <td>
                                                                    <?php echo htmlentities($row->AppTitle); ?>
                                                                </td>

                                                                <td>
                                                                    <?php echo htmlentities($row->Student); ?>
                                                                </td>
                                                                <td>
