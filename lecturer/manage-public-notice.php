...
if (strlen($_SESSION['sturecmsaid'] == 0)) {
  ...
  if (isset($_GET['delid'])) {
    ...
    $sql = "delete from tblmessage where ID=:rid";
  ...
  <head>
    ...
    <title>MMP Management System ||| View Message</title>
    <!-- plugins:css -->
                ...
                <div class="card">
                  <div class="card-body">
                    <div class="d-sm-flex align-items-center mb-4">
                      <h4 class="card-title mb-sm-0">View Message</h4>
                      <a href="#" class="text-dark ml-auto mb-3 mb-sm-0"> View all Messages</a>
                    </div>
                    <div class="table-responsive border rounded p-1">
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          $ret = "SELECT ID FROM tblmessage";
                          $query1 = $dbh->prepare($ret);
                          $query1->execute();
                          $results1 = $query1->fetchAll(PDO::FETCH_OBJ);
                          $total_rows = $query1->rowCount();
                          $total_pages = ceil($total_rows / $no_of_records_per_page);
                          $sql = "SELECT ID, TitleMsg, ByStudent, CreationDate from tblmessage";
                          $query = $dbh->prepare($sql);
                          $query->execute();
                          ...
                          if ($query->rowCount() > 0) {
                            foreach ($results as $row) { ?>
                              <tr>
                                ...
                                <td>
                                  <?php echo htmlentities($row->TitleMsg); ?>
                                </td>
                                <td>
                                  <?php echo htmlentities($row->ByStudent); ?>
                                </td>
                                <td>
